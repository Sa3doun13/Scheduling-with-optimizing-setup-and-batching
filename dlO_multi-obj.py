...
class Application(clingo.Application):
    ...
    def main(self, prg, files):
        ...
        while i <= NUM_OF_TIME_WINDOWS:
            ...
            print('Time of the 1st Call : {} '.format(time_used))
            # ************************ Setup/Batching Optimization [Weak constraints] ************************
            time_used = 0
            bound = lastbound
            self.weak_const(prg, bound)
            ...
            toc = time.time()
            time_used += (toc - tic)
            print('Time of the 2nd Call : {} '.format(time_used))
